...
여기서 말하는 “기억”의 범위를 먼저 정하자. 이 장에서 문제 삼는 기억은 모델이 사전학습으로 익힌 일반 지식이 아니다. 예를 들어 한국어 문법, JavaScript 문법, HTTP의 기본 개념처럼 모델 가중치에 들어 있는 지식은 넓은 의미에서 모델의 파라미터 기억이라고 부를 수 있다.

이 장에서 말하는 기억은 더 좁다.

```text
사용자별·프로젝트별·대화별로 새로 생긴 정보를
호출 사이에 계속 유지하는 영속적 기억
```

예를 들면 이런 것들이다.

```text
사용자가 예전에 만든 Telegram 봇 구조
이 repo에서 정한 집필 계획
특정 프로젝트의 테스트 명령
사용자의 선호와 작업 방식
```

일반적인 LLM 추론 호출은 이런 사용자별 상태를 모델 파라미터에 새로 저장하지 않는다. 애플리케이션이나 하네스가 다시 제공해야 한다.
...
이 한 줄이 이 책의 첫 번째 기둥이다. 엄밀히 말하면 출력은 모델 파라미터, 디코딩 설정, 샘플링, tool schema, 응답 형식 제약에도 영향을 받는다. 하지만 여기서는 하네스가 제어할 수 있는 입력 부분을 보기 위해 단순화해서 `context`라고 부른다.
...
```text
사용자별 기억처럼 보이는 정보는 대개
하네스가 현재 context에 다시 제공한 것이다
```
...
여기서 context란 단순히 “사용자가 방금 쓴 질문”만 뜻하지 않는다. 또한 반드시 하나로 이어 붙인 거대한 문자열만 뜻하지도 않는다.

이 책에서는 편의상 context를 더 넓게 쓴다.

```text
모델의 출력을 조건화하는 모델 가시 상태 전체
```

구현에 따라 이것은 역할이 구분된 메시지 배열일 수도 있고, 별도 `tools` 필드에 들어가는 JSON Schema일 수도 있고, tool result 항목, conversation state, compaction 결과, 환경 메타데이터일 수도 있다.

즉 context는 “프롬프트 문자열”보다 넓은 말이다.

보통 여러 층의 정보가 함께 들어간다.
...
HTTP는 기본적으로 stateless하다. 요청 라인만 보면 서버는 사용자를 식별할 수 없다.

```text
GET /me
```

이 한 줄만으로는 “이 사람이 누구인지”가 없다. 하지만 실제 HTTP 요청 전체에는 Cookie나 Authorization 헤더가 포함될 수 있다.
...
| 웹 서버 | LLM 에이전트 |
|---|---|
| 요청 라인/body | user message |
| cookie/session ID/auth header | thread ID / user ID / memory key |
...
아래 항목은 제품 표면, 설정, 활성화된 기능, 현재 세션 상태에 따라 포함될 수 있다. 항상 전부 들어간다는 뜻은 아니다.
...
호스티드 제품의 최종 모델 요청과 모든 내부 orchestration은 완전히 공개되어 있지 않다. 다만 Codex CLI와 공식 문서에서 `AGENTS.md`, 환경 context, skill, hook 처리 같은 주요 표면은 상당 부분 확인할 수 있다. 공부할 때 중요한 것은 이 관점이다.
...
모델이 강할수록 긴 context를 잘 읽고, 단서를 잘 연결하고, 요약에 남아 있는 정보를 더 잘 활용한다. 그래서 더 기억력이 좋아 보인다.

하지만 요약은 보통 손실 압축이다. 요약에서 빠진 ID, 수치, 날짜, 예외 조건을 모델이 정확히 되살릴 수 있다고 기대하면 안 된다. 빠진 정보는 복원되는 것이 아니라 추정된다.
...
너무 적게 넣으면 필요한 배경이 빠진다. 하지만 너무 많이 넣는 것도 공짜가 아니다. 관련 정보의 활용 성능이 떨어질 수 있고, 비용과 지연이 늘며, 오래되거나 충돌하는 정보가 섞일 가능성도 커진다.
...
조금 더 정확히 말하면 좋은 하네스는 다음을 함께 설계한다.
...
2. 이 장에서 말하는 기억은 사용자별·프로젝트별 정보를 호출 사이에 유지하는 영속적 기억이다. 사전학습된 일반 지식과는 구분한다.
3. 기억처럼 보이는 것은 대개 history, summary, memory, tool result가 모델 가시 상태에 들어왔기 때문이다.
4. 웹 서버에서 session middleware가 request.user를 붙이듯, agent harness는 LLM request에 필요한 정보를 붙인다.
5. 좋은 agent는 많은 정보를 넣는 시스템이 아니라, 필요한 정보를 고르고, 배치하고, 압축하고, 출처와 최신성을 관리하는 시스템이다.
6. 하네스 엔지니어링의 첫 질문은 “모델이 뭘 봤는가?”다.
```
