...
하지만 이 다섯 개를 제대로 만들고 관찰하면, 앞 장들의 개념이 손에 잡힌다.
...
## Lab 3. Mini Mode Plugin

세 번째 lab은 Mini Mode Plugin이다.
...
Codex 공식 문서 기준으로 plugin은 skills, apps, MCP server를 묶어 배포할 수 있는 단위다. plugin은 lifecycle hooks도 bundle할 수 있다. skill은 reusable workflow의 authoring format이고, plugin은 그것을 설치 가능한 형태로 나눠주는 distribution unit에 가깝다.
...
4. Mini Mode Plugin은 skill/hook/plugin surface 차이를 실험하게 한다.
...
7. 좋은 lab은 작고, 재현 가능하고, 관찰 결과가 남는다.
8. Harness engineering 실력은 “모델이 이상하다”를 구체적 원인으로 쪼개는 능력에서 나온다.
```
